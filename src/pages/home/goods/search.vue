<template>
    <div class="page">
        <div class="search-top">
            <div class="search-header">
                <div class="back" @click="$router.go(-1)"></div>
                <div class="search-wrap" @click="isShowSearch.show=true">
                    <div class="search-icon"></div>
                    <div class="search-text">{{keyword}}</div>
                </div>
                <div class="screen-btn" @click="isScreen=true">筛选</div>
            </div>
            <div class="order-main">
                <div :class="{'order-item':true, active: isPriceOrder}" @click="selectPrice()">
                    <div class="order-text">综合</div>
                    <div class="order-icon"></div>
                    <ul class="order-menu" v-show="isPriceOrder">
                        <li :class="{active: item.active}" v-for="(item, index) in priceOrderList" :key="index"
                            @click="selectPriceOrder(index)">{{item.title}}
                        </li>
                    </ul>
                </div>
                <div :class="{'order-item':true, active: isSaleOrder}" @click="selectSales()">
                    <div class="order-text">销量</div>
                </div>
            </div>
        </div>
        <div class="goods-main">
            <div class="goods-list" v-for="(item,index) in searchData" :key="index" @click="$router.push('/goods/details?gid='+item.gid)">
                <div class="image"><img src="../../../assets/images/common/lazyImg.jpg" alt="" :data-echo="item.image">
                </div>
                <div class="goods-content">
                    <div class="goods-title">{{item.title}}</div>
                    <div class="price">¥{{item.price}}</div>
                    <div class="sales">销量<span>{{item.sales}}</span>件</div>
                </div>
            </div>
            <div class="no-data" v-show="searchData.length <= 0">没有相关商品！</div>
        </div>
        <div class="mask" v-show="isScreen" @click="isScreen=false"></div>
        <div :class="isScreen ? 'screen move' : 'screen unmove'" ref="screen">
            <div>
                <div class="attr-wrap">
                    <div class="attr-title-wrap" @click="isClassifyHide=!isClassifyHide">
                        <div class="attr-name">分类</div>
                        <div :class="{'attr-icon':true,up:isClassifyHide}"></div>
                    </div>
                    <div class="item-wrap" v-show="!isClassifyHide">
                        <div :class="{item:true, active: item.active}" v-for="(item, index) in classifyGoods"
                             :key="index" @click="selectClassify({index: index})">{{item.title}}
                        </div>
                    </div>
                </div>
                <div style="width: 100%; height: 1px; background-color: rgb(239, 239, 239);"></div>
                <div class="attr-wrap">
                    <div class="attr-title-wrap" @click="isPriceRangeHide=!isPriceRangeHide">
                        <div class="attr-name">价格区间</div>
                        <div class="price-wrap" @click.stop>
                            <div class="price-input"><input type="tel" placeholder="最低价" :value="minPrice"
                                                            @input="SET_MINPRICE({minPrice: $event.target.value})">
                            </div>
                            <div class="price-line"></div>
                            <div class="price-input"><input type="tel" placeholder="最高价" :value="maxPrice"
                                                            @input="SET_MAXPRICE({maxPrice: $event.target.value})">
                            </div>
                        </div>
                        <div :class="{'attr-icon':true,up:isPriceRangeHide}"></div>
                    </div>
                    <div class="item-wrap" v-show="!isPriceRangeHide">
                        <div :class="{item:true, active: item.active}" v-for="(item, index) in priceRangeData"
                             :key="index" @click="SELECT_PRICE_RANGE({index: index})">{{item.price1}}-{{item.price2}}
                        </div>
                    </div>
                </div>
                <div style="width: 100%; height: 0.3rem; background-color: rgb(239, 239, 239);"></div>
                <div>
                    <div class="attr-wrap" v-for="(item, index) in attrs" :key="index">
                        <div class="attr-title-wrap" @click="item.isHide =!item.isHide">
                            <div class="attr-name">{{item.title}}</div>
                            <div :class="{'attr-icon':true, up: item.isHide}"></div>
                        </div>
                        <div class="item-wrap" v-show="!item.isHide">
                            <div :class="{item:true, active: item2.active}" v-for="(item2, index2) in item.param"
                                 :key="index2" @click="SELECT_ATTR({index: index, index2: index2})">{{item2.title}}
                            </div>
                        </div>
                    </div>

                    <div style="width: 100%; height: 1px; background-color: rgb(239, 239, 239);"></div>
                </div>
                <div style="width: 100%; height: 1.2rem;"></div>
            </div>
            <div class="handel-wrap">
                <div class="item">共<span>{{total}}</span>件</div>
                <div class="item reset" @click="reset()">全部重置</div>
                <div class="item sure" @click="submit()">确定</div>
            </div>
        </div>
        <my-search :showSearch="isShowSearch" :isLocal="true"></my-search>
    </div>
</template>

<script>
    import {mapState, mapActions, mapMutations} from "vuex"
    import MySearch from "../../../components/search/search"
    import IScroll from "../../../assets/js/libs/iscroll"
    import UpRefresh from "../../../assets/js/libs/uprefresh"

    export default {
        name: "search",
        data() {
            return {
                keyword: this.$route.query.keyword ? this.$route.query.keyword : "",
                isShowSearch: {show: false},
                isPriceOrder: false,
                priceOrderList: [
                    {otype: "all", title: "综合", active: true},
                    {otype: "up", title: "价格从低到高", active: false},
                    {otype: "down", title: "价格从高到低", active: false},
                ],
                isSaleOrder: false,
                isScreen: false,
                isClassifyHide: false,
                isPriceRangeHide: false
            }
        },
        computed: {
            ...mapState({
                classifyGoods: state => state.goods.classifyGoods,
                priceRangeData: state => state.search.priceRangeData,
                minPrice: state => state.search.minPrice,
                maxPrice: state => state.search.maxPrice,
                attrs: state => state.search.attrs,
                searchData: state => state.search.searchData,
                cid: state => state.search.cid,
                params: state => state.search.params,
                total: state => state.search.total
            })
        },
        created() {
            this.otype = "all";
            this.getClassify({
                success: () => {
                    this.$nextTick(() => {
                        this.myScroll.refresh();
                    });
                }
            });
            this.pullUp = new UpRefresh();
            this.reset();
            this.init();
            this.getAttrs({
                keyword: this.keyword, success: () => {
                    this.$nextTick(() => {
                        this.myScroll.refresh();
                    });
                }
            });
        },
        mounted() {
            this.$refs["screen"].addEventListener("touchmove", this.preventScroll);
            this.myScroll = new IScroll(this.$refs["screen"], {
                scrollX: false,
                scrollY: true,
                preventDefault: false
            });
        },
        methods: {
            ...mapActions({
                getClassify: "goods/getClassify",
                selectClassify: "search/selectClassify",
                getSearch: "search/getSearch",
                getSearchPage: "search/getSearchPage",
                getAttrs: "search/getAttrs",
                reset: "search/reset"
            }),
            ...mapMutations({
                "SELECT_PRICE_RANGE": "search/SELECT_PRICE_RANGE",
                "SET_MINPRICE": "search/SET_MINPRICE",
                "SET_MAXPRICE": "search/SET_MAXPRICE",
                "SELECT_ATTR": "search/SELECT_ATTR",
                "SET_PID": "search/SET_PID"
            }),
            selectPrice() {
                this.isPriceOrder = !this.isPriceOrder;
                this.isSaleOrder = false;
            },
            selectPriceOrder(index) {//价格排序
                if (this.priceOrderList.length > 0) {
                    for (let i = 0; i < this.priceOrderList.length; i++) {
                        if (this.priceOrderList[i].active) {
                            this.priceOrderList[i].active = false;
                            break;
                        }
                    }
                    this.priceOrderList[index].active = true;
                    this.$set(this.priceOrderList, index, this.priceOrderList[index]);
                    this.otype = this.priceOrderList[index].otype;
                    this.init();
                }
            },
            selectSales() {//销量排序
                this.isSaleOrder = true;
                this.isPriceOrder = false;
                for (let i = 0; i < this.priceOrderList.length; i++) {
                    if (this.priceOrderList[i].active) {
                        this.priceOrderList[i].active = false;
                        break;
                    }
                }
                this.otype = "sales";
                this.init();
            },
            //禁用touchmove事件
            preventScroll(e) {
                e.preventDefault();
            },
            init() {
                let jsonParams = {
                    keyword: this.keyword,
                    otype: this.otype,
                    cid: this.cid,
                    price1: this.minPrice,
                    price2: this.maxPrice,
                    param: JSON.stringify(this.params)
                }
                this.getSearch({
                    ...jsonParams, success: (pageNum) => {
                        this.$nextTick(() => {//图片懒加载
                            this.$utils.lazyImg();
                        });
                        this.pullUp.init({
                            "curPage": 1,
                            "maxPage": parseInt(pageNum),
                            "offsetBottom": 100
                        }, (page) => {
                            //console.log(page);
                            this.getSearchPage({...jsonParams, page: page});
                        });
                    }
                });
            },
            //确定
            submit() {
                this.isScreen = false;
                this.SET_PID();
                this.init();
            }
        },
        components: {
            MySearch
        },
        beforeRouteUpdate(to, from, next) {
            this.keyword = to.query.keyword;
            this.isPriceOrder = false;

            if (this.priceOrderList.length > 0) {
                for (let i = 0; i < this.priceOrderList.length; i++) {
                    if (this.priceOrderList[i].active) {
                        this.priceOrderList[i].active = false;
                        break;
                    }
                }
            }
            this.otype = "all";
            this.isSaleOrder = false;
            this.priceOrderList[0].active = true;
            this.reset();
            this.init();

            this.getAttrs({
                keyword: this.keyword, success: () => {
                    this.$nextTick(() => {
                        this.myScroll.refresh();
                    });
                }
            });
            next();
        },
        beforeDestroy() {
            this.$refs["screen"].removeEventListener("touchmove", this.preventScroll);
            this.pullUp.uneventSrcoll();
        }
    }
</script>

<style scoped>
    .page {
        width: 100%;
        height: auto;
        background-color: #fff;
        overflow: hidden
    }

    .search-top {
        width: 100%;
        position: fixed;
        z-index: 10;
        left: 0;
        top: 0;
        background-color: #fff
    }

    .search-header {
        width: 100%;
        height: .8rem;
        border-bottom: 1px solid #efefef;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        -webkit-box-pack: justify;
        -ms-flex-pack: justify;
        justify-content: space-between;
        -webkit-justify-content: space-between;
        -webkit-box-align: center;
        -ms-flex-align: center;
        align-items: center;
        -webkit-align-items: center
    }

    .search-header .back {
        width: .8rem;
        height: .8rem;
        background-image: url(data:image/png;base64,);
        background-size: 100%;
        background-repeat: no-repeat;
        background-position: 50%
    }

    .search-header .search-wrap {
        width: 75%;
        background: #eaeaea;
        height: 80%;
        border-radius: .1rem;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        -webkit-box-align: center;
        -ms-flex-align: center;
        align-items: center;
        -webkit-align-items: center
    }

    .search-header .search-wrap .search-icon {
        width: .5rem;
        height: .5rem;
        background-image: url(data:image/png;base64,);
        background-repeat: no-repeat;
        background-size: 100%;
        background-position: 50%;
        margin-left: .1rem;
        margin-right: .2rem
    }

    .search-header .search-wrap .search-text {
        width: 90%;
        height: auto;
        font-size: .32rem
    }

    .search-header .screen-btn {
        width: auto;
        height: auto;
        font-size: .32rem;
        margin-right: .1rem
    }

    .search-top .order-main {
        width: 100%;
        height: .8rem;
        border-bottom: 1px solid #efefef;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        -webkit-box-align: center;
        -ms-flex-align: center;
        align-items: center;
        -webkit-align-items: center
    }

    .search-top .order-main .order-item {
        width: auto;
        height: auto;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        margin-left: .4rem;
        position: relative;
        z-index: 1
    }

    .search-top .order-main .order-item .order-text {
        font-size: .32rem
    }

    .search-top .order-main .order-item.active .order-text {
        color: #fda208
    }

    .search-top .order-main .order-item .order-icon {
        width: .35rem;
        height: .35rem;
        background-image: url(data:image/png;base64,);
        background-size: 100%;
        background-position: 50%;
        background-repeat: no-repeat;
        margin-left: .1rem
    }

    .search-top .order-main .order-item.active .order-icon {
        background-image: url(data:image/png;base64,);
        background-size: 100%;
        background-position: 50%;
        background-repeat: no-repeat
    }

    .search-top .order-main .order-item .order-menu {
        width: 688%;
        height: auto;
        background-color: #fff;
        position: absolute;
        z-index: 1;
        left: -35%;
        top: .63rem
    }

    .search-top .order-main .order-item .order-menu li {
        width: 95%;
        height: .8rem;
        border-bottom: 1px solid #efefef;
        font-size: .28rem;
        line-height: .8rem;
        padding-left: .4rem
    }

    .search-top .order-main .order-item .order-menu li.active {
        color: #fda208
    }

    .goods-main {
        width: 100%;
        height: auto;
        margin-top: 2rem
    }

    .goods-main .goods-list {
        width: 100%;
        height: 2rem;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        -webkit-box-pack: justify;
        -ms-flex-pack: justify;
        justify-content: space-between;
        -webkit-justify-content: space-between;
        overflow: hidden;
        margin-bottom: .4rem
    }

    .goods-main .goods-list .image {
        width: 2rem;
        height: 2rem;
        margin-left: .1rem;
        overflow: hidden;
        text-align: center
    }

    .goods-main .goods-list .image img {
        width: auto;
        height: auto;
        max-height: 100%
    }

    .goods-main .goods-list .goods-content {
        width: 67%;
        height: 98%;
        border-bottom: 1px solid #ccc
    }

    .goods-main .goods-list .goods-title {
        width: 95%;
        height: .8rem;
        font-size: .28rem;
        overflow: hidden
    }

    .goods-main .goods-list .price {
        font-size: .32rem;
        color: #f93036;
        margin-top: .1rem
    }

    .goods-main .goods-list .sales {
        font-size: .24rem;
        color: #969696;
        margin-top: .1rem
    }

    .goods-main .goods-list .sales span {
        color: #fda208
    }

    .mask {
        width: 100%;
        height: 100%;
        position: fixed;
        z-index: 99;
        left: 0;
        top: 0;
        background-color: rgba(0, 0, 0, .3)
    }

    .screen {
        width: 80%;
        height: 100%;
        position: fixed;
        z-index: 100;
        right: 0;
        top: 0;
        background-color: #fff;
        transform: translateX(100%);
        -webkit-transform: translateX(100%);
        overflow: hidden
    }

    .screen.move {
        transform: translateX(0);
        -webkit-transform: translateX(0)
    }

    .screen.move, .screen.unmove {
        -webkit-transition: -webkit-transform .3s;
        transition: -webkit-transform .3s;
        transition: transform .3s;
        transition: transform .3s, -webkit-transform .3s;
        -webkit-transition: transform .3s
    }

    .screen.unmove {
        transform: translateX(100%);
        -webkit-transform: translateX(100%)
    }

    .screen .attr-wrap {
        width: 100%
    }

    .screen .attr-wrap .attr-title-wrap {
        width: 94%;
        height: .8rem;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        -webkit-box-pack: justify;
        -ms-flex-pack: justify;
        justify-content: space-between;
        -webkit-justify-content: space-between;
        -webkit-box-align: center;
        -ms-flex-align: center;
        align-items: center;
        -webkit-align-items: center;
        padding-left: .2rem;
        padding-right: .2rem
    }

    .screen .attr-wrap .attr-title-wrap .attr-name {
        font-size: .32rem
    }

    .screen .attr-wrap .attr-title-wrap .attr-icon {
        width: .4rem;
        height: .4rem;
        background-image: url(data:image/png;base64,);
        background-size: 100%;
        background-repeat: no-repeat;
        background-position: 50%
    }

    .screen .attr-wrap .attr-title-wrap .attr-icon.up {
        background-image: url(data:image/png;base64,);
        background-size: 100%;
        background-repeat: no-repeat;
        background-position: 50%
    }

    .screen .attr-wrap .attr-title-wrap .price-wrap {
        width: auto;
        height: auto;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        -webkit-box-align: center;
        -ms-flex-align: center;
        align-items: center;
        -webkit-align-items: center;
        margin-left: 18%
    }

    .screen .attr-wrap .attr-title-wrap .price-wrap .price-input {
        width: 1rem;
        height: .4rem;
        border: 1px solid #efefef
    }

    .screen .attr-wrap .attr-title-wrap .price-wrap .price-input input {
        width: 100%;
        height: 89%;
        font-size: .28rem
    }

    .screen .attr-wrap .attr-title-wrap .price-wrap .price-line {
        width: .4rem;
        height: 1px;
        background-color: #efefef;
        margin-left: .1rem;
        margin-right: .1rem
    }

    .screen .attr-wrap .item-wrap {
        width: 100%;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex;
        -ms-flex-wrap: wrap;
        flex-wrap: wrap;
        -webkit-flex-wrap: wrap
    }

    .screen .attr-wrap .item-wrap .item {
        width: 30%;
        height: .64rem;
        background-color: #efefef;
        text-align: center;
        line-height: .64rem;
        overflow: hidden;
        font-size: .28rem;
        border-radius: .1rem;
        margin-left: 2%;
        margin-right: 1%;
        margin-bottom: .2rem
    }

    .screen .attr-wrap .item-wrap .item.active {
        color: #fff;
        background-color: #fda208
    }

    .screen .handel-wrap {
        width: 100%;
        height: 1rem;
        background-color: #fff;
        border-top: 1px solid #efefef;
        position: absolute;
        z-index: 1;
        left: 0;
        bottom: 0;
        display: -webkit-box;
        display: -ms-flexbox;
        display: flex;
        display: -webkit-flex
    }

    .screen .handel-wrap .item {
        width: auto;
        height: 100%;
        font-size: .28rem;
        text-align: center;
        line-height: 1rem;
        -webkit-box-flex: 1;
        -ms-flex: 1;
        flex: 1;
        -webkit-flex: 1
    }

    .screen .handel-wrap .item span {
        color: #fda208
    }

    .screen .handel-wrap .item.reset {
        background-color: #efefef
    }

    .screen .handel-wrap .item.sure {
        background-color: #fda208;
        color: #fff
    }
</style>