<template>
    <div>
        <div class="details-header">
            <div class="back" @click="$router.go(-1)"></div>
            <div class="tab-wrap">
                <div :class="{'tab-name':true, active:itemActive}" @click="$router.replace('/goods/details?gid='+gid)">商品
                </div>
                <div :class="{'tab-name':true, active:contentActive}"
                     @click="$router.replace('/goods/details/content?gid='+gid)">详情
                </div>
                <div :class="{'tab-name':true, active:reviewActive}" @click="$router.replace('/goods/details/review?gid='+gid)">
                    评价
                </div>
            </div>
            <div id="cart-icon" class="cart-icon" @click="$router.push('/cart?from=goods_details')">
                <div class="spot" v-show="$store.state.cart.cartData.length>0"></div>
            </div>
        </div>
        <div class="sub-page">
            <router-view></router-view>
        </div>
    </div>
</template>

<script>
    export default {
        name: "goods-details",
        data() {
            return {
                gid: this.$route.query.gid ? this.$route.query.gid : "",
                itemActive: true,
                contentActive: false,
                reviewActive: false
            }
        },
        created() {
            this.changeTabStyle(this.$route.name);
        },
        methods: {
            changeTabStyle(name) {
                switch (name) {
                    case "goods-item":
                        this.itemActive = true;
                        this.contentActive = false;
                        this.reviewActive = false;
                        break;
                    case "goods-content":
                        this.itemActive = false;
                        this.contentActive = true;
                        this.reviewActive = false;
                        break;
                    case "goods-review":
                        this.contentActive = false;
                        this.itemActive = false;
                        this.reviewActive = true;
                        break;
                    default:
                        this.itemActive = true;
                        this.contentActive = false;
                        this.reviewActive = false;
                        break;
                }
            }
        },
        beforeRouteUpdate(to, from, next) {
            this.changeTabStyle(to.name);
            next();
        }
    }
</script>

<style scoped>
    .details-header {
        width: 100%;
        height: 1rem;
        background-color: #fff;
        display: flex;
        align-items: center;
        justify-content: space-between;
        position: fixed;
        z-index: 10;
        left: 0;
        top: 0
    }

    .details-header .back {
        width: .8rem;
        height: .8rem;
        background-image: url(data:image/png;base64,);
        background-repeat: no-repeat;
        background-position: 50%;
        background-size: 100%
    }

    .details-header .tab-wrap {
        width: 70%;
        height: 100%;
        display: flex;
        justify-content: center;
    }

    .details-header .tab-wrap .tab-name {
        width: auto;
        height: 96%;
        overflow: hidden;
        line-height: 1rem;
        text-align: center;
        margin-left: 9%;
        margin-right: 9%;
        font-size: .36rem
    }

    .details-header .tab-wrap .tab-name.active {
        border-bottom: 2px solid #fda208
    }

    .details-header .cart-icon {
        width: .8rem;
        height: .8rem;
        background-image: url(data:image/png;base64,);
        background-repeat: no-repeat;
        background-position: 50%;
        background-size: 100%;
        margin-right: .2rem;
        position: relative;
        z-index: 1
    }

    .details-header .cart-icon .spot {
        width: .2rem;
        height: .2rem;
        background-color: #cc0004;
        border-radius: 100%;
        position: absolute;
        z-index: 1;
        right: 0;
        top: .15rem
    }

    .sub-page {
        width: 100%
    }
</style>